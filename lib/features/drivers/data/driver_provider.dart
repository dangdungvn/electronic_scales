...
@riverpod
class DriverList extends _$DriverList {
  @override
  Future<List<Driver>> build() async {
    return await _fetchDrivers();
  }
  ...
  Future<List<Driver>> _fetchDrivers() async {
    try {
      ...
      throw Exception('Lỗi khi tải danh sách tài xế: ${e.toString()}');
    }
  }
  ...
  Future<void> deleteDriver(String id) async {
    try {
      ...
      final dio = Dio();
      final baseUrl = 'http://${station.ip}:${station.port}';
      print('Deleting driver with ID: $id');
      final response = await dio.post(
        '$baseUrl/scm/XoaTaiXe',
        options: Options(
          headers: {
            'Authorization': permissions.token,
            'Content-Type': 'application/json; charset=utf-8',
